...
namespace Bilog
{
    public class MetaWeblogService : IMetaWeblogProvider
    {
        ...
        private readonly ApplicationDbContext _BilogContext;
        ...
        public MetaWeblogService(
            ...
            ApplicationDbContext bilogContext,
            GeneralSettingsService generalSettingsService,
            UserManager<ApplicationUser> userManager)
        {
            _environment = environment;
            _httpContextAccessor = httpContextAccessor;
            _BilogContext = bilogContext;
        ...
        public async Task<UserInfo> GetUserInfoAsync(string key, string username, string password)
        {
            ...
            {
                var Blogger = await _userManager.FindByEmailAsync(username);
        ...
        public async Task<Post> GetPostAsync(string postid, string username, string password)
        {
            ...
            if (await IsValidMetaWeblogUserAsync(username, password))
            {
                ...
                var BlogPost = await _BilogContext.Blogs
                ...
                objPost.categories = _BilogContext.Categorys
        ...
        public async Task<Post[]> GetRecentPostsAsync(string blogid, string username, string password, int numberOfPosts)
        {
            ...
            if (await IsValidMetaWeblogUserAsync(username, password))
            {
                ...
                var BlogPosts = await _BilogContext.Blogs
                    .Include(x => x.BlogCategory)
                    .Where(x => x.BlogUserName == username)
                    .Take(numberOfPosts)
        ...
        public async Task<string> AddPostAsync(string blogid, string username, string password, Post post, bool publish)
        {
            ...
            if (await IsValidMetaWeblogUserAsync(username, password))
            {
                try
                {
                    ...
                    _BilogContext.Add(objBlogs);
                    _BilogContext.SaveChanges();
        ...
        public async Task<bool> DeletePostAsync(string key, string postid, string username, string password, bool publish)
        {
            if (await IsValidMetaWeblogUserAsync(username, password))
            {
                ...
                {
                    _BilogContext.Blogs.Remove(ExistingBlogs);
                    _BilogContext.SaveChanges();
                }
                else
                {
        ...
        public async Task<CategoryInfo[]> GetCategoriesAsync(string blogid, string username, string password)
        {
            ...
            if (await IsValidMetaWeblogUserAsync(username, password))
            {
                var Categorys = await _BilogContext.Categorys.ToListAsync();
        ...
        private List<BlogCategory> GetBlogCategories(Blogs objBlogs, IEnumerable<string> blogCatagories)
        {
            ...
            {
                // Get the Category
                var Category = _BilogContext.Categorys
